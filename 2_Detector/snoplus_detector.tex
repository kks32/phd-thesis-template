...
One can show that the differential cross-section associated with Rayleigh scattering of unpolarised light, $\sigma_{\textrm{Ray}}\left(\theta,\phi\right)$,  is given by~\cite{}: % cite something that gets rayleigh scattering formula
\begin{equation}
...
% $\sigma_{\textrm{Ray}}\left(\theta,\phi\right)$ corresponds to the differential cross-section for one scattering particle. Given an entire medium of these particles, one can then find the mean distance one would expect a photon should travel before undergoing Rayleigh scattering, $l_{\textrm{Ray}}$~\cite{}: % cite Rayleigh scattering length formula
% \begin{equation}
%     l_{\textrm{Ray}} = \left[\frac{128\pi^{5}a^{6}}{3\lambda^{4}}\frac{1}{V}\left(\frac{n_{\textrm{par}}^{2}-1}{n_{\textrm{par}}^{2}+2}\right)^{2}\right]^{-1},
% \end{equation}
% where $V$ is the 
