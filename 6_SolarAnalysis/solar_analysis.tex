...
Table~\ref{tab:MC_expected_rates_constraints} shows the derived expected number of events for the \beight{} signal using the global fit flux constraint prior, over the livetime of the dataset used in this analysis. 
...
the vast majority of external backgrounds which start from a substantial distance away from the AV do not generate $\gamma$s that make it into the scintillator. As a result, an enormous amount of computational resources could be wasted on simulating events that never get into the ROI. To work around this: 
\begin{itemize}
    \item PMT $\beta-\gamma$ events are modelled as \SI{2.6}{\MeV} $\gamma$s generated at a radius of \SI{6.2}{\m} (just outside the AV), pointing radially inwards.
    \item Simulations of background events in the external water and ropes are restricted from starting beyond certain maximum radii depending on the specific process, such that there remains a negligible impact on the fraction of simulated events that actually deposit any energy into the scintillator~\cite{}. % cite Ana Sofia's DocDB #5154
\end{itemize} 
